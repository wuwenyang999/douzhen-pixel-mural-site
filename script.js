...
function renderMine() {
  ...
  const sectionCounts = grid.cells.reduce((counts, cell) => ({ ...counts, [cell.code]: (counts[cell.code] || 0) + 1 }), {});
  ownedContent.innerHTML = `<section class="material-summary"><div><small>色卡品牌</small><b>${summary.brand}</b></div><div><small>整幅总颗数</small><b>${summary.totalBeads.toLocaleString()} 颗</b></div><div><small>分区数量</small><b>${summary.sectionCount} 区</b></div><div><small>使用颜色</small><b>${summary.colours.length} 色</b></div></section><article class="owned-card"><img src="assets/azure-dragon.png" alt="苍龙镇海"><div class="owned-copy"><p class="eyebrow">巨幅壁画 <span>${percent}% 完成</span></p><h3>苍龙镇海</h3><p>按分区逐块完成，进度会保留在当前浏览器。</p><div class="progress"><i style="width:${percent}%"></i></div><div class="sections">${sections.map((section) => { const id = section.slice(0, 3); return `<button class="${completed.includes(id) ? 'done' : ''}" data-section="${id}">${completed.includes(id) ? '✓ ' : ''}${section}</button>`; }).join('')}</div></div></article><section class="grid-paper"><div><p class="eyebrow">分区色块图 <span>${activeSection} · 10 × 10</span></p><h3>${sections.find((section) => section.startsWith(activeSection))}</h3><p>每个色块代表一颗拼豆；悬停可查看对应 MARD 色号。</p><div class="grid-tabs">${sections.map((section) => { const id = section.slice(0, 3); return `<button class="${id === activeSection ? 'active' : ''}" data-view-section="${id}">${section}</button>`; }).join('')}</div></div><div class="bead-grid" style="grid-template-columns:repeat(${grid.columns},1fr)">${grid.cells.map((cell, index) => `<span style="background:${cell.hex}" title="${cell.code} · ${activeSection}-${String(index + 1).padStart(2, '0')}"></span>`).join('')}</div></section><section class="colour-list"><div class="colour-heading"><p class="eyebrow">MARD 291 采购清单 <span>全图 / 当前分区</span></p><p>整幅图 ${summary.totalBeads} 颗；每种色按全图数量备料，再以当前分区数量分装。</p></div><div class="colour-table">${summary.colours.map((colour) => `<div><i style="background:${colour.hex}"></i><span><b>${colour.code}</b><small>${colour.name} · ${colour.hex}</small></span><em>${colour.count} 颗<small>全图</small></em><strong>${sectionCounts[colour.code] || 0} 颗<small>${activeSection}</small></strong></div>`).join('')}</div></section>`;
